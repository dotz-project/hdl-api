...
use app\drivers\karafka\CloudKarafka;

class KarafkaClusterRequestConsumer implements ConsumerInterface
{
    public function execute(AMQPMessage $msg) {
        $data = json_decode($msg->body,true);
        try{
            try{
                $_comp =  Components::findOne(['name'=>'Karafka', 'id' => $data['component_id']]);
            ...
            try{
                $karafka = new CloudKarafka($_env->alias,[
                    "conf" => $_comp->driver_params,
                    "body" => $data
                ]);
                $result = $karafka->createCluster();
                if ($result['status'] == 200) {
                    $data['_request_'] = json_decode($result['body'], true);
                    \Yii::$app->rabbitmq->getProducer('KARAFKA.CLUSTER.PROCESS.PRODUCER')->publish(json_encode($data), 'ORCHID', 'KARAFKA.CLUSTER.PROCESS');
